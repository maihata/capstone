...
map_summary_df <- df2 %>%
  ...
  summarise(
    ...
    # metric used everywhere (matches app.R "largest" logic)
    disparity_spread = high_log - low_log,

    # use the same metric for the map shading
  ...
  mutate(
    ...
    hover_text = paste0(
      "State: ", state,
      "<br>Exit category: ", unname(cat_labels[category]),
      "<br>Highest: ", high_race,
      " (OR ", sprintf("%.2f", high_or),
      ", ln ", sprintf("%.2f", high_log), ")",
      "<br>Lowest: ", low_race,
      " (OR ", sprintf("%.2f", low_or),
      ", ln ", sprintf("%.2f", low_log), ")",
      "<br>Largest within-category gap (ln OR): ",
      sprintf("%.2f", disparity_spread),
      ifelse(
        unreliable_state,
        "<br>Data flag: Caution (suppressed/small/extreme)",
        ""
      )
    )
  )
...
welcome_map_df <- map_summary_df %>%
 ...
 hover_text = paste0(
  "State: ", state,
  "<br>Largest disparity category: ", unname(cat_labels[category]),
  "<br>Highest: ", high_race, " (OR ", sprintf("%.2f", high_or), ", ln ", sprintf("%.2f", high_log), ")",
  "<br>Lowest: ",  low_race,  " (OR ", sprintf("%.2f", low_or),  ", ln ", sprintf("%.2f", low_log),  ")",
  "<br>Max |ln(OR)| (across categories): ", sprintf("%.2f", map_value),
  "<br>Flag: ", ifelse(unreliable_state, "Caution: suppressed/small/extreme", "OK")
)
  )
